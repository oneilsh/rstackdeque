...
#' @details An rpqueue supports efficient insert into the back with insert_back and 
#' (returning a version of
#' the queue with the new element), peek into the front with peek_front
#' (returing the data stored at the front element), and removal with
#' without_front (returning a version with the front
#'  element removed). 
#' 
...
#' @param x rpqueue to get the head of.
#' @param ... arguments to be passed to or from other methods.
#' @param n number of elements to get.
#' @return potentially smaller rpqueue with the front n elements kept.
#' @examples 
#' q <- rpqueue()
#' q <- insert_back(q, "a")
#' q <- insert_back(q, "b")
...
#' 
#' @description Converts the elements of an rpqueue into rows of a dataframe, if this is reasonable.
#' 
...
#' @return a data.frame with the first row(s) the previous front of the rpqueue, last row(s) previous back of the rpqueue.
#' @examples 
#' q <- rpqueue()
...
#' @return vector of length 1, with the number of elements of the rpqueue.
#' @examples
#' q <- rpqueue()
#' q <- insert_back(q, "a")
#' print(length(q))         # 1
...
#' 
#' @description A method for keeping the pair of lazy lists approximately equal
...
#' 
#' @param rpqueue rpqueue to rotate
#' @param acclazylist lazy list that serves as the incremental accumulator.
...
#' @description Used for keeping an rpqueue O(1) as it is used.
